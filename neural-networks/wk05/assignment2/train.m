...
learning_rate = 0.1;  % Learning rate; default = 0.1.
                        %  1 epoch  train CE: 0.001=4.432, 0.1=3.963, 10=4.698
                        % 10 epochs train CE: 0.001=4.379, 0.1=2.540, 10=4.6
momentum = 0.9;    % Momentum; default = 0.9.
                     % 0.0 train CE: 3.994  valid CE: 3.954  test CE: 3.958
                     % 0.5 train CE: 3.327  valid CE: 3.255  test CE: 3.253
                     % 0.9 train CE: 2.713  valid CE: 2.711  test CE: 2.721
numhid1 = 50;      % Dimensionality of embedding space; default = 50.
numhid2 = 200;     % Number of units in hidden layer; default = 200.
                     % 5 dimension,  100 hidden train CE=2.806 valid CE=2.823 test CE=2.827
                     % 50 dimension,  10 hidddn train CE=3.003 valid CE=3.015 test CE=3.012
                     % 50 dimension, 200 hidden train CE=2.533 valid CE=2.606 test CE=2.617
                     % 100 dimension,  5 hidden train CE=3.232 valid CE=3.241 test CE=3.239
...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    %% EMBEDDING LAYER.
    for w = 1:numwords
       word_embedding_weights_gradient = word_embedding_weights_gradient + expansion_matrix(:, input_batch(w, :)) * (back_propagated_deriv_2(1 + (w - 1) * numhid1 : w * numhid1, :)');
    end
    ...
    if mod(m, show_validation_CE_after) == 0
      ...
      CE = -sum(sum(expanded_valid_target .* log(output_layer_state + tiny))) / datasetsize;
...
CE = -sum(sum(expanded_test_target .* log(output_layer_state + tiny))) / datasetsize;
